...
def get_summary_card(month_name, initial, final, in_pj, in_pf, out_pj, out_pf):
    return [
        ...
        ["", ""],
        ["ENTRADAS", ""],
        ["PJ", format_curr(in_pj)],
        ["PF", format_curr(in_pf)],
        ["", ""],
        ["SAÍDAS", ""],
        ["PJ", format_curr(out_pj)],
        ["PF", format_curr(out_pf)],
        ["", ""],
...
def restructure():
    ...
    # 2. Unified Data for "CONTA CORRENTE BB"
    jan_data = [
    ...
    # 3. Summary Cards for "RESUMO MENSAL"
    # Column B: Label, Column C: Value
    ...
    # Place cards side by side (JAN at B2, FEV at F2)
    # I'll create a 2D array for the summary sheet
    summary_layout = [[""]*10 for _ in range(30)]
    for r_idx, r in enumerate(card_jan):
        summary_layout[r_idx+1][1] = r[0]
        summary_layout[r_idx+1][2] = r[1]
    for r_idx, r in enumerate(card_fev):
        summary_layout[r_idx+1][5] = r[0]
        summary_layout[r_idx+1][6] = r[1]
    ...
    # 5. Formatting
    # Clear formatting on CONTA CORRENTE BB first
    requests = [
        # Zebra on Lançamentos using BandedRanges
        {
            "addBandedRange": {
                "bandedRange": {
                    "range": {"sheetId": lan_id, "startRowIndex": 0, "endRowIndex": 100, "startColumnIndex": 0, "endColumnIndex": 8},
                    "rowProperties": {
                        "headerColor": {"red": 0.0, "green": 0.3, "blue": 0.1},
        ...
        # Yellow Cards for Resumo (JAN)
        {
            "repeatCell": {
                "range": {"sheetId": res_id, "startRowIndex": 1, "endRowIndex": 15, "startColumnIndex": 1, "endColumnIndex": 3},
                "cell": {"userEnteredFormat": {"backgroundColor": {"red": 1.0, "green": 1.0, "blue": 0.0}, "borders": {"bottom": {"style": "SOLID"}, "top": {"style": "SOLID"}, "left": {"style": "SOLID"}, "right": {"style": "SOLID"}}}},
                "fields": "userEnteredFormat(backgroundColor,borders)"
            }
        },
        # Yellow Cards for Resumo (FEV)
        {
            "repeatCell": {
                "range": {"sheetId": res_id, "startRowIndex": 1, "endRowIndex": 15, "startColumnIndex": 5, "endColumnIndex": 7},
                "cell": {"userEnteredFormat": {"backgroundColor": {"red": 1.0, "green": 1.0, "blue": 0.0}, "borders": {"bottom": {"style": "SOLID"}, "top": {"style": "SOLID"}, "left": {"style": "SOLID"}, "right": {"style": "SOLID"}}}},
                "fields": "userEnteredFormat(backgroundColor,borders)"
            }
        }
    ...
    service.spreadsheets().batchUpdate(spreadsheetId=SPREADSHEET_ID, body={'requests': requests}).execute()
    print("Professional Restructuring Complete.")
