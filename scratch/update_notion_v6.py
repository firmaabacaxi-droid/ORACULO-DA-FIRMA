...
if "error" in update_res:
    ...
    print("Proposta atualizada com sucesso para R$ 65.000 (Versão 6).")

print("\n2. Buscando itens de orçamento vinculados...")
# Vamos fazer uma query no banco de dados BUDGET_DB_ID para encontrar os registros correspondentes
query_payload = {
    "filter": {
        "property": "Proposta",
        "relation": {
...
for page in results:
    ...
    if "Edição e finalização de vídeos instrucionais (ENAP)" in title:
        ...
        if "error" in res:
            ...
            print(f"Item '{title}' atualizado com sucesso para R$ 13.983.")
            
    elif "NF 7,28%" in title:
        print(f"Atualizando item '{title}'...")
        update_payload = {
            "properties": {
                "Valor unitário": {"number": 4405.00},
                "Total": {"number": 4405.00}
            }
        }
        res = request_notion(f"https://api.notion.com/v1/pages/{page_id}", update_payload, method="PATCH")
        if "error" in res:
            ...
            print(f"Item '{title}' atualizado com sucesso para R$ 4.405.")
