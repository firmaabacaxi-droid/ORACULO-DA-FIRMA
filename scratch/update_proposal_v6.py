...
doc_path = r"c:\Users\User\Documents\ORACULO - FIRMA ABACAXI\output\propostas\BrasilParticipativo_proposta_v2 lipe.docx"
...
if idx_cronograma != -1:
    ...
    p_ref_body = doc.paragraphs[10] # paragrafo normal de corpo garantido com runs
    p_ref_bullet = doc.paragraphs[12] # bullet reference (usa EM DASH)
    ...
    # Em v2 lipe, p_cronograma.runs[0] é o bullet e p_cronograma.runs[1] é "  Cronograma". Vamos renomear para "  Cronograma do Documentário"
    p_cronograma.runs[1].text = "  Cronograma do Documentário"
...
idx_pagamento = -1
for i, p in enumerate(doc.paragraphs):
    if "Pagamento:" in p.text:
        idx_pagamento = i
        break

if idx_pagamento != -1:
    p_pagamento = doc.paragraphs[idx_pagamento]
    p_ref_body = doc.paragraphs[10] # garantido com runs
    p_ref_bullet = doc.paragraphs[12] # bullet reference
    p_ref_head = doc.paragraphs[idx_cronograma] if idx_cronograma != -1 else doc.paragraphs[9]
    
    # Deletar o parágrafo de pagamento original
    p_parent = p_pagamento._p.getparent()
    p_parent.remove(p_pagamento._p)
    
    # Encontrar a Validade para inserir antes dela
    idx_validade = -1
    for i, p in enumerate(doc.paragraphs):
        if "Validade desta proposta:" in p.text:
            idx_validade = i
            break
            
    if idx_validade != -1:
        p_validade = doc.paragraphs[idx_validade]
        ...
        r_pay_text1 = p_pay_b1.add_run("Escopo 1 — Documentário Principal: 50% do valor do documentário (R$ 25.012,50) faturado mediante a entrega da primeira versão do roteiro documental, e 50% (R$ 25.012,50) faturado mediante a entrega do produto final (arquivo master aprovado).")
        ...
        print("[ERROR] Nao encontrou paragrafo de Validade")
else:
    print("[ERROR] Nao encontrou paragrafo de Pagamento")
...
idx_investimento = -1
for i, p in enumerate(doc.paragraphs):
    if "Investimento" in p.text and p.text.strip().startswith(chr(8226)):
        idx_investimento = i
        break

if idx_investimento != -1:
    p_invest = doc.paragraphs[idx_investimento]
    p_invest_sub1 = doc.paragraphs[idx_investimento + 1] # parágrafo entre título e tabela
    
    # Inserir cabeçalho do Escopo 1 antes da tabela 1
    p_escopo1_head = p_invest_sub1.insert_paragraph_before()
...
print("[OK] Tabela 1 identificada (Documentario)")

# Agora vamos criar a Tabela 2 e Tabela 3.
# Elas devem ser inseridas depois do parágrafo de nota de rodapé da tabela 1:
idx_nota = -1
for i, p in enumerate(doc.paragraphs):
    if "*A diária de filmagem" in p.text:
        idx_nota = i
        break

if idx_nota != -1:
    p_nota = doc.paragraphs[idx_nota]
    ...
    update_cell_text_preserve_format(table3.rows[1].cells[1], "R$ 46.630,00")
    update_cell_text_preserve_format(table3.rows[1].cells[2], "R$ 3.395,00")
    update_cell_text_preserve_format(table3.rows[1].cells[3], "R$ 50.025,00")
    ...
    update_cell_text_preserve_format(table3.rows[3].cells[3], "R$ 65.025,00")
