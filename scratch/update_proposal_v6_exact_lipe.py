...
if idx_cronograma != -1:
    ...
    # Adicionar bullet com formatação correta
    ...
    if len(p_ref_head.runs) > 1:
        copy_run_format(p_ref_head.runs[1], r_text_head)
    else:
        copy_run_format(p_ref_head.runs[0], r_text_head)
    r_text_head.bold = True
    ...
    if len(p_ref_bullet.runs) > 1:
        copy_run_format(p_ref_bullet.runs[1], r_text1)
    else:
        copy_run_format(p_ref_bullet.runs[0], r_text1)
    ...
    if len(p_ref_bullet.runs) > 1:
        ...
        copy_run_format(p_ref_bullet.runs[0], r_text2)
    ...
    if len(p_ref_bullet.runs) > 1:
        ...
        copy_run_format(p_ref_bullet.runs[0], r_text3)
    ...
    if len(p_ref_bullet.runs) > 1:
        ...
        copy_run_format(p_ref_bullet.runs[0], r_text4)
    ...
    if len(p_ref_bullet.runs) > 1:
        ...
        copy_run_format(p_ref_bullet.runs[0], r_text5)
...
if p_pagamento and p_validade:
    ...
    if len(p_ref_head.runs) > 1:
        ...
        copy_run_format(p_ref_head.runs[0], r_text_pay)
    r_text_pay.bold = True
    ...
    # 2.3 Bullet 1: Documentário (com valores exatos de R$ 50.025,00)
    ...
    if len(p_ref_bullet.runs) > 1:
        ...
        copy_run_format(p_ref_bullet.runs[0], r_pay_text1)
    ...
    if len(p_ref_bullet.runs) > 1:
        ...
        copy_run_format(p_ref_bullet.runs[0], r_pay_text2)
...
# Inserir cabeçalho do Escopo 1 antes da tabela 1 (XML Insert)
...
idx_nota = -1
for i, p in enumerate(doc.paragraphs):
    if "*a diária de filmagem" in p.text.lower() or "*a diária de" in p.text.lower() or "* inclui dire" in p.text.lower():
        p_nota = p
        idx_nota = i
        break
...
if idx_nota != -1:
    ...
    # Row 1: Documentário (com valores exatos de R$ 50.025,00)
